...
public class SimpleHttpInvokerRequestExecutor
    ...
    */
    ...
    protected RemoteInvocationResult doExecuteRequest(
        HttpInvokerClientConfiguration config, String invocationStr)
        throws IOException, ClassNotFoundException {
        HttpURLConnection con = openConnection(config);
        prepareConnection(con);
        OutputStream out =con.getOutputStream();
        // DataOutputStream.writeBytes将字符串中的16位的unicode字符以8位的字符形式写道流里面
        out.write(invocationStr.getBytes("UTF-8")); 
...
//        writeRequestBody(config, con, baos);
    ...
    */
    protected HttpURLConnection openConnection(
        HttpInvokerClientConfiguration config) throws IOException {
        URLConnection con = new URL(config.getServiceUrl()).openConnection();

        if (!(con instanceof HttpURLConnection)) {
            throw new IOException("Service URL [" + config.getServiceUrl() +
                "] is not an HTTP URL");
        }
    ...
    */
    protected void prepareConnection(HttpURLConnection connection,
        ...
        connection.setRequestProperty(HTTP_HEADER_CONTENT_LENGTH,
        ...
        if (isAcceptGzipEncoding()) {
            connection.setRequestProperty(HTTP_HEADER_ACCEPT_ENCODING,
                ENCODING_GZIP);
        }
    }
...
//                Integer.toString(contentLength));
    ...
    */
    protected void writeRequestBody(HttpInvokerClientConfiguration config,
        HttpURLConnection con, ByteArrayOutputStream baos)
        throws IOException {
    ...
    */
    protected InputStream readResponseBody(
        ...
        if (isGzipResponse(con)) {
        	...
        	System.out.println(XmlBeanConvert.toXml(con));
    ...
    */
    protected boolean isGzipResponse(HttpURLConnection con) {
        ...
        encodingHeader.toLowerCase().contains(ENCODING_GZIP));
    }
}
